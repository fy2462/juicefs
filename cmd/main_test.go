...
func TestHandleSysMountArgs(t *testing.T) {
	var cases = []struct {
		...
		{
			[]string{"/mount.juicefs", "memkv://", "/jfs", "-o", "remote-cache=mock,remote-cache-nodes=127.0.0.1:9000,remote-cache-transport=rdma,remote-cache-timeout=25ms,remote-cache-replicas=2,remote-cache-fail-threshold=4,remote-cache-node-cooldown=3s,remote-cache-probe-interval=500ms,remote-cache-probe-timeout=20ms,remote-cache-fill-local=true,remote-cache-fill-remote=false"},
			"juicefs mount -d --remote-cache=mock --remote-cache-nodes=127.0.0.1:9000 --remote-cache-transport=rdma --remote-cache-timeout=25ms --remote-cache-replicas=2 --remote-cache-fail-threshold=4 --remote-cache-node-cooldown=3s --remote-cache-probe-interval=500ms --remote-cache-probe-timeout=20ms --remote-cache-fill-local=true --remote-cache-fill-remote=false memkv:// /jfs",
			false,
		},
		{
