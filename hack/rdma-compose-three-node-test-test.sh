...
assert_file_contains "$CLIENT_SCRIPT" "--metrics"
assert_file_contains "$CLIENT_SCRIPT" "metric_value"
assert_file_contains "$CLIENT_SCRIPT" "assert_metric_gt"
assert_file_contains "$CLIENT_SCRIPT" "assert_metric_eq"
assert_file_contains "$CLIENT_SCRIPT" "juicefs_remote_cache_gets_total"
assert_file_contains "$CLIENT_SCRIPT" "juicefs_remote_cache_fallbacks_total"
assert_file_contains "$CLIENT_SCRIPT" "juicefs_remote_cache_node_down"
assert_file_contains "$CLIENT_SCRIPT" "juicefs_remote_cache_node_skips_total"
assert_file_contains "$CLIENT_SCRIPT" "read-node-health-fallback"
assert_file_contains "$CLIENT_SCRIPT" "payload2.txt"
assert_file_contains "$CLIENT_SCRIPT" "docker compose three-node read survives one L2 node and L3 outage"
assert_file_contains "$CLIENT_SCRIPT" "docker compose three-node marks stopped L2 down and skips it"
...
assert_file_contains "$SCRIPT" "read-node-health-fallback"
