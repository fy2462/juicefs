...
grep -F "hack/rdma-cache-stress.sh" "$workflow" >/dev/null ||
  fail "rdma-cache workflow does not trigger on the stress script"
...
grep -F "hack/rdma-native-mounted-failover-smoke-test-test.sh" "$workflow" >/dev/null ||
  fail "rdma-cache workflow does not trigger on the native mounted failover smoke guard script"

grep -F "./hack/rdma-native-mounted-failover-smoke-test-test.sh" "$workflow" >/dev/null ||
  fail "rdma-cache workflow does not run the native mounted failover smoke guard script"
