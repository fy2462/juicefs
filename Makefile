...
test.rdma-native-mounted-mock:
	@test -n "$${OPEN_RDMA_DRIVER:-}" || (echo "OPEN_RDMA_DRIVER is required for test.rdma-native-mounted-mock" >&2; exit 2)
	./hack/rdma-native-mounted-smoke-test.sh --mock-rdma "$${OPEN_RDMA_DRIVER}"

test.rdma-native-mounted-failover-mock:
	@test -n "$${OPEN_RDMA_DRIVER:-}" || (echo "OPEN_RDMA_DRIVER is required for test.rdma-native-mounted-failover-mock" >&2; exit 2)
	./hack/rdma-native-mounted-failover-smoke-test.sh --mock-rdma "$${OPEN_RDMA_DRIVER}"
...
test.rdma-native-mock-stress:
	...
	JFS_RDMA_SMOKE_OPS=$${JFS_RDMA_STRESS_OPS:-1} JFS_RDMA_SMOKE_CONCURRENCY=$${JFS_RDMA_STRESS_CONCURRENCY:-1} ./hack/rdma-native-smoke-test.sh --mock-rdma "$${OPEN_RDMA_DRIVER}"
